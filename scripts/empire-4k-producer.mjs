...
async function render4KVideo(topicDir, topicName, audioPath, subtitlePath) {
  ...
  if (!isVercel) {
    console.log(`🚀 [GPU FFmpeg] Hardware-accelerated render...`);
    const gpuResult = renderWithGPU(topicDir, topicName, audioPath, subtitlePath, FFMPEG);
    if (gpuResult) return gpuResult;
  }

  // ── PRIORITY 2: CPU FFmpeg + effects — free, unlimited, proven working ─
  console.log(`🎬 [CPU FFmpeg] Rendering with Ken Burns effects...`);
  ...
  try {
    ...
    console.log(`✅ [CPU FFmpeg] Video rendered: ${outputFile}`);
    return outputFile;
  } catch (e) {
    console.error(`❌ [CPU FFmpeg] Failed: ${e.message}`);

    // ── PRIORITY 3: Shotstack — restricted free (20/month limit) ──────────
    if (hasShotstackKey) {
      console.log(`☁️ [Shotstack] Cloud render (restricted free, 20/month)...`);
      const shotstackResult = await renderWithShotstack(topicDir, topicName, audioPath);
      if (shotstackResult) return shotstackResult;
      console.warn(`⚠️ [Shotstack] Failed...`);
    }

    // ── PRIORITY 4: CPU FFmpeg simple — free, unlimited, final fallback ───
    console.log(`🔄 [CPU Simple] Final fallback: no effects...`);
    ...
    const fallbackComplex = simpleConcatFilters + ';' + concatPart + ';[vout]copy[vfinal]';
    ...
    try {
      ...
      console.log(`✅ [CPU Simple] Fallback render succeeded`);
      return outputFile;
    } catch (fallbackError) {
      console.error(`❌ All rendering methods exhausted: ${fallbackError.message}`);
    }
  }
  return null;
}
