...
description: Audit the active repo, MCP servers, plugins, connectors, env surfaces, and harness setup, then recommend the highest-value native skills, hooks, agents, and operator workflows. Use when the user wants help setting up Claude Code or understanding what capabilities are actually available in their environment.
---
...
This is this project-native answer to setup-audit plugins. It does not modify files unless the user explicitly asks for follow-up implementation.
...
- Comparing official marketplace plugins against native coverage
...
- Prefer native workflows over generic "install another plugin" advice when crucible can reasonably own the surface.
...
- Separate three things clearly:
  ...
  - available but not wrapped well in this project
...
- existing skills already relevant to the workspace

If a surface exists only as a primitive, call that out. Example:

- "Stripe is available via connected app, but this project lacks a billing-operator skill"
- "Google Drive is connected, but there is no native Google Workspace operator workflow"
...
2. whether crucible already has parity
3. whether crucible only has primitives
4. whether crucible is missing the workflow entirely

### Phase 3: Turn Gaps Into Decisions

For every real gap, recommend the correct native shape:

| Gap Type | Preferred crucible Shape |
|----------|---------------------|
...
2. **Parity**
   - where already matches or exceeds the benchmark
3. **Primitive-only gaps**
   - tools exist, but this project lacks a clean operator skill
4. **Missing integrations**
   - capability not available yet
5. **Top 3-5 next moves**
   - concrete native additions, ordered by impact
...
- If crucible already has a strong primitive, propose a wrapper skill instead of inventing a brand-new subsystem.

## Good Outcomes

- The user can immediately see what is connected, what is missing, and what crucible should own next.
