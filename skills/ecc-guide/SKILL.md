...
Answer from current files, not memory. crucible changes quickly, so hard-coded catalog counts, feature lists, and install instructions go stale.

When this project repository is available, inspect the relevant files before giving a concrete answer:
...
Use `/project-init` when the user wants crucible configured for a target repo. The expected sequence is:
