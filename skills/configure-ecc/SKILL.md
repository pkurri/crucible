...
Before any installation, clone the latest crucible source to `/tmp`:
...
Set `SKILLS_ROOT=/tmp/everything-claude-code` as the source for all subsequent copy operations.

If the clone fails (network issues, etc.), use `AskUserQuestion` to ask the user to provide a local path to an existing crucible clone.
...
```
Question: "Where should crucible components be installed?"
Options:
...
`claude-api` is an Anthropic canonical skill. Install it from [`anthropics/skills`](https://github.com/anthropics/skills) when you want the official Claude API workflow instead of an crucible-bundled copy.
...
cp -R "$SKILLS_ROOT/.agents/skills/<skill-name>" "$TARGET/skills/"

# Niche skills live under skills/
cp -R "$SKILLS_ROOT/skills/<skill-name>" "$TARGET/skills/"
```
...
```bash
# Common rules
cp -r $SKILLS_ROOT/rules/common $TARGET/rules/common

# Language-specific rules (preserve per-language directories)
cp -r $SKILLS_ROOT/rules/typescript $TARGET/rules/typescript   # if selected
cp -r $SKILLS_ROOT/rules/python $TARGET/rules/python            # if selected
cp -r $SKILLS_ROOT/rules/golang $TARGET/rules/golang            # if selected
```
...
**Critical**: Only modify files in the installation target (`$TARGET/`), NEVER modify files in the source crucible repository (`$SKILLS_ROOT/`).
