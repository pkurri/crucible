...
- Sentence rhythm: (e.g. mostly short, one long every few lines; or steady
  medium)
- Signature openers: (lines/phrases you tend to start with)
- Punctuation habits: (e.g. you use `..` as a soft pause; you never use em
  dashes)
...
- Always: (e.g. one concrete number per post; first-person; end on a real
  question)
- Never: (e.g. no engagement-bait, no "in today's fast-paced world", no naming
  competitors)
...
on-brand via a pixel-exact overlay. All optional; leave blank to skip the
overlay.
