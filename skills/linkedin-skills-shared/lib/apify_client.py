...
Actors used (all no-cookies, public, "$1-$5 per 1,000 results"):
  ...
  - apimaestro/linkedin-profile-detail
      Fetch a profile's headline, About/summary, experience, education,
      skills, and location by username or profile URL. Use for
      linkedin-profile-optimizer intake so the audit starts from live data
      instead of a manual paste. Does not cover photo/banner (image
      composition still needs a screenshot or manual description).
...
class ApifyClient:
    ...
    PROFILE_DETAIL_ACTOR = "apimaestro~linkedin-profile-detail"
    ...
    # ---- Profile detail (own or others' public profile) -------------------
    ...
    def fetch_profile(
        ...
        *,
        ...
        """Return a public profile's headline, About, experience, education,
        skills, and location.

        Args:
            username: LinkedIn username, full profile URL, or URN
                (e.g. "prasadkurri" or "https://www.linkedin.com/in/prasadkurri").
            include_email: If True, also extract a publicly listed email.
            force_refresh: Bypass cache.

        Returns:
            The actor's raw profile record. Photo/banner composition is not
            covered — those still need a screenshot or manual description.
        """
        items = self._run_sync(
            self.PROFILE_DETAIL_ACTOR,
            {"username": username, "includeEmail": include_email},
            force_refresh=force_refresh,
        )
        if not items:
            raise ApifyError(f"no profile returned for {username}")
        return items[0]
