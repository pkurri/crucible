...
**Opt-in (crucible):** The `codescene` block in `mcp-configs/mcp-servers.json` is a template only. plugin installs do not auto-enable bundled MCP servers. Copy the entry into your config only if you want it. You can exclude it during skills install/sync with `DISABLED_MCPS=codescene,...`.
...
Same triggers as **When to Use** above — this heading is what crucible uses for skill auto-activation.
...
## Pairing with this toolkit

| crucible skill / flow | Code Health MCP role |
|------------------|----------------------|
...
**Context tip:** crucible recommends keeping MCP count low. Enable `codescene` when doing substantive edits; disable when not needed.
