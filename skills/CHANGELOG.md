...
- Wired optional `tool-ui-ux-pro-max` enrichment into `tool-design-style-selector` and `workflow-crucible` to make design specs more concrete (palette/typography/UX guardrails).
...
- Fixed `tool-ui-ux-pro-max/SKILL.md` line endings to LF (CRLF can cause some skill loaders to miss the skill entirely).
- Expanded UI/UX trigger phrases (added `UIUX`/`uiux` and common UX design synonyms) to improve routing accuracy.
- Made `tool-ui-ux-pro-max` enrichment default when installed in `tool-design-style-selector` and `workflow-crucible`.
- Added a “Brainstorm-lite + demo moment” kickoff to `workflow-crucible` to avoid shipping a basic, hard-to-demo MVP.
- Added a required scope confirmation step to `workflow-crucible` so core steps are not silently marked as skipped.
...
- Standardized skill docs to be English-only (removed non-English trigger phrases and symbols).
