...
media. Runs on any AI coding agent that can read Markdown skill files and run
Python.

## When this runs

- User says "add an image", "make an illustration", "make a quote-card", or "add
  a cover" for a post.
...
1. **Pick the kind** (maps to an aspect ratio automatically):
   - `wide` / `link` (1200:628) - link-preview / single feed image (default for
     a text post)
   - `portrait` / `carousel` / `quote` (4:5) - feed portrait, carousel slide,
     quote-card
   - `square` (1:1) - generic Override with an explicit `aspect_ratio="w:h"`
     when needed.
...
3. **Apply brand overlay (if profile has it).** Read
   `../../linkedin-skills-shared/references/voice-profile.md` §6 Brand assets.
   If a handle, brand color, or logo is set, pass an `overlay` so the text/logo
   is composited pixel-exact (crisp even on a cheap model):
   ...
   prompt) so it renders sharp: `kind="quote"`,
   `overlay={"text": "<hook>", ...}`.

4. **Model choice.** Default `nano-banana-2` (balanced, ~$0.08). The overlay
   handles text, so a cheap base model is fine. Only reach for
   `gemini-pro-image` when the user wants premium art. Never silently upgrade
   the tier.
5. **Show + confirm.** Present the returned `url` and `cost`. On approval,
   attach it when publishing:
   `publish("post", draft_text, target_url, media_urls=[r["url"]])`.
6. **Manual mode.** If `r["backend"] == "manual"`, show `r["message"]` (the
   drafted prompt + aspect) and ask for a pasted URL to attach.
...
`refine` edits by `img_...` id (not URL), inherits the source shape/tier when
you omit `aspect_ratio`/`resolution`, and is cheaper + more consistent than a
fresh generation. Chain it as many times as needed.
...
- `lib.available_models()` returns live pricing/latency when you need to show
  it.
...
- Respect the user's cost: default to the cheap model + 1K resolution unless
  asked.
