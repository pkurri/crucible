...
Use this skill when the user suspects this project Tools GitHub App is burning cost, over-creating PRs, bypassing usage limits, or routing free users into premium analysis paths.

This is a focused operator workflow for the sibling [crucible-Tools](../../crucible-Tools) repo. It is not a generic billing skill and it is not a repo-wide code review pass.

## Skill Stack

Pull these native skills into the workflow when relevant:
...
- user says crucible Tools burn rate, PR recursion, over-created PRs, usage-limit bypass, or premium-model leakage
- the task is in the sibling `crucible-Tools` repo and depends on webhook handlers, queue workers, usage reservation, PR creation logic, or paid-gate enforcement
...
- work in the sibling `crucible-Tools` repo, not in `everything-claude-code`
...
- switch into the sibling `crucible-Tools` repo
