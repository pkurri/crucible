...
name: mcp-supabase
description:
  'Execute database operations via Supabase MCP (query/write/migration/logs/type
  generation). Triggers:
...
Interact with Supabase database via MCP tools, execute queries, writes,
...
- Need to perform "database actions" on Supabase: query/statistics/export, write
  (after confirmation), migration (DDL), type generation, query logs/advisors

**Does not apply to:**

- Need to complete "integration implementation" in Next.js project (env/client
  code/minimal data access layer/project structure)  
  → Use `workflow-crucible` (Step 6: Supabase integration) for project-side
  setup; this skill only handles DB-side actions and gates

**Called by:**

- `workflow-crucible` uses this skill as DB operation foundation;
  `workflow-crucible` handles project-side integration, this skill handles
  DB-side actions and security gates

## Postgres Best Practices (Bundled)

crucible vendors Supabase's Postgres best practices inside the `supabase` skill
(install `supabase` alongside this skill if you want these references available
locally):
...
When integrating database operations into multi-step workflows, persist all
...
- Input: `01-input/goal.md` (requirements), `01-input/context.json` (known
  tables/fields/IDs)
- Plan: `03-plans/sql.md` (SQL to execute; write operations must be written here
  before confirmation)
- Output: `05-final/result.md` (conclusion + key numbers + SQL + truncated
  results)
...
6. **DDL via migration**: Schema changes must use `apply_migration`,
   `execute_sql` cannot run DDL directly
...
Default to information_schema / `generate_typescript_types` as source of truth.
