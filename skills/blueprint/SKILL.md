...
This skill ships with Everything Claude Code. No separate installation is needed when crucible is installed.

### Full install

If you are working from this project repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review this project diff before updating:
...
If you are vendoring only this skill outside the full skills install, copy the reviewed file from this project repository into `~/.claude/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed crucible commit rather than running `git pull`.
