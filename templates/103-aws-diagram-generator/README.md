...
AI-powered AWS architecture diagram generator. Describe an architecture in
natural language, get an editable `.drawio` file with proper AWS icons.
Refine through conversation.

```text
You: "3-tier web app with ALB, ECS, RDS PostgreSQL, and ElastiCache"
Generator: [generates diagram in ~10s] -> editable .drawio file

You: "Add CloudFront CDN in front of the ALB"
Generator: [patches diagram in ~5s] -> updated .drawio file
```
...
- **Natural language -> diagram** — describe what you want, get a
  `.drawio` file
- **Conversational edits** — refine via chat, edits take ~5s via JSON
  patches
- **178 AWS service icons** — official `mxgraph.aws4` stencil library
  with verified icon catalog
- **Deterministic rendering** — style guide enforced in code, not in
  the prompt
- **Editable output** — open in draw.io, export to PNG/SVG/PDF
- **Smart layout** — Graphviz-powered with source/sink rank pinning
  and auxiliary node placement
- **Error-safe icons** — broken stencil blocklist with automatic
  fallbacks
- **User-scoped storage** — each user's diagrams are isolated

## How It Works

Two-phase architecture that separates what to draw (LLM) from how to draw it
(renderer):

1. **LLM generates a compact JSON spec** (~400 tokens) describing nodes,
   edges, and clusters
2. **Deterministic renderer** maps AWS service types to draw.io icons,
   computes layout via Graphviz, and emits valid draw.io XML
3. **For edits**, the LLM generates a JSON patch (RFC 6902) instead of
   regenerating everything — ~5s vs ~30s

The LLM never touches draw.io XML. Style guide compliance (icon sizes, fonts,
colors, container hierarchy) is enforced in code, not in the prompt.
...
- src/                          Backend (Python)
  ...
  - renderer/
    - schema.py                 JSON spec dataclasses + validation
    - icons.py                  178 AWS service -> draw.io icon mapping
    - layout.py                 Graphviz layout with rank pinning
    - emitter.py                draw.io XML emitter
  - tools/
    - render_drawio.py          @tool: render spec -> draw.io XML -> S3
    - load_diagram.py           @tool: load existing diagram from S3
...
- frontend/                     Frontend (React + TypeScript)
  - src/
    ...
    - api.ts                    API client - job polling, CRUD, usage
    - auth.ts                   Cognito PKCE OAuth2 flow
    - styles.ts                 Design system
- infra/                        Infrastructure (CloudFormation)
  - cfn/
    - api.yaml                  Internal API Gateway + Lambda + Cognito
    - api-public.yaml           Public API Gateway + DynamoDB usage table
    - cognito-public.yaml       Public Cognito pool
    - frontend-public.yaml      CloudFront + S3 for the frontend
    - ecr.yaml                  ECR repository
  - lambda/
    - handler.py                Lambda handler - jobs, usage, user scoping
    - Dockerfile                Lambda image (Python 3.13 + Graphviz)
...
- Python 3.13+
- Node.js 18+
- [Graphviz](https://graphviz.org/download/) (`brew install graphviz` on
  macOS)
...
Deploy your own stack with `infra/deploy.sh` (backend) and
`infra/deploy-public-frontend.sh` (frontend) — edit the CloudFormation
templates under `infra/cfn/` for your own domain and AWS account first.
...
- **Broken icons blocklist** — known-broken stencil names (e.g.,
  `dynamodb_table`) are mapped to working alternatives
- **Safe fallback** — unknown service types render as `general_AWScloud`
  icon (always visible) instead of guessing stencil names
- **Legacy name mapping** — renamed services use their old stencil names
  (e.g., OpenSearch -> `elasticsearch_service`)
